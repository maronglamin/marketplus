...
type RepProductReportNavigationProp = NativeStackNavigationProp<AppStackParamList, 'RepProductReport'>;
...
export function RepProductReport() {
  const navigation = useNavigation<RepProductReportNavigationProp>();
  ...
  const loadProducts = useCallback(async (pageNum: number, shouldRefresh = false) => {
    try {
      ...
      setError(null);
      console.log('Fetching rep products:', { pageNum, shouldRefresh });
      ...
      if (shouldRefresh) {
        ...
        setProducts(prev => {
          ...
          const newProducts = productActivities.filter(p => !existingIds.has(p.id));
      ...
      console.log('Updated rep products state:', { 
        totalProducts: shouldRefresh ? productActivities.length : 'appended', 
  ...
  const handleLoadMore = useCallback(() => {
    if (!loadingMore && hasMore && !loading) {
      console.log('Loading more rep products, current page:', page);
      loadProducts(page + 1);
    }
  ...
  const renderProductCard = (product: RepProduct) => {
        ...
        <View style={styles.productInfo}>
          ...
          <View style={styles.productMeta}>
            <View style={styles.metaItem}>
              ...
              <Text style={styles.metaText}>{product.salesRepName}</Text>
            </View>
          </View>
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Loading products from sales reps...</Text>
        </View>
      </SafeAreaView>
    );
  }
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.container}>
          <View style={styles.header}>
            ...
            <Text style={styles.title}>Rep Products Report</Text>
        ...
        >
          {products.length === 0 ? (
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>No products found from sales reps</Text>
              <Text style={styles.emptySubtext}>Products uploaded by your sales reps will appear here</Text>
            </View>
          ) : (
            <>
