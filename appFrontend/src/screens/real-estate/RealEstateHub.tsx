...
const BUY_TYPES = [
  { type: 'HOME_SALE' as const, title: 'Homes for Sale', subtitle: 'Browse residential properties', icon: 'business-outline' as const, color: '#059669' },
  { type: 'LAND_SALE' as const, title: 'Land for Sale', subtitle: 'Plots & development land', icon: 'map-outline' as const, color: '#D97706' },
];
  ...
  const openAgentFlow = async () => {
    ...
    try {
      ...
      if (data?.agent || data?.application?.status === 'APPROVED') {
        ...
        navigation.navigate('BecomePropertyAgent');
      }
    } catch {
      navigation.navigate('BecomePropertyAgent');
    } finally {
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        <View style={styles.header}>
          ...
          <View style={styles.headerText}>
            <Text style={styles.headerTitle}>Stays & Realty</Text>
            <Text style={styles.headerSubtitle}>Hotels, rentals, homes & land</Text>
          </View>
        </View>
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          >
            ...
            <View style={styles.agentBannerText}>
              <Text style={styles.agentBannerTitle}>Become a Property Agent</Text>
          ...
          <Text style={[styles.sectionTitle, { marginTop: 24 }]}>Buy</Text>
          <View style={styles.grid}>
            {BUY_TYPES.map((item) => (
              <TouchableOpacity
                key={item.type}
