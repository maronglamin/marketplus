...
import { settlementService, type BankAccount, type Wallet, type AvailableRealEstateEarnings } from '../../services/settlementService';
import { useAuth } from '../../contexts/AuthContext';

type RealEstateSettlementRequestNavigationProp = NativeStackNavigationProp<AppStackParamList, 'RealEstateSettlementRequest'>;

interface AvailableRealEstateEarning {
  currency: string;
  amount: number;
...
export function RealEstateSettlementRequest() {
  const navigation = useNavigation<RealEstateSettlementRequestNavigationProp>();
  ...
  const loadData = async () => {
    try {
      ...
      console.error('Error loading real estate settlement data:', e);
  ...
  const handleSubmitSettlement = async () => {
    ...
    try {
      ...
      await settlementService.createSettlementRequest({
        ...
        includedPropertyBookingIds: (earnings.bookings || [])
          .map(b => b.id || '')
          .filter((id) => !!id),
        totalPropertyBookingsCount: ((earnings.bookings || [])
          .map(b => b.id || '')
          .filter((id) => !!id)).length,
      });
      ...
      console.error('Error submitting real estate settlement request:', error);
