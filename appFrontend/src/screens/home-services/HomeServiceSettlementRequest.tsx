...
import { settlementService, type BankAccount, type Wallet, type AvailableHomeServiceEarnings } from '../../services/settlementService';
import { useAuth } from '../../contexts/AuthContext';

type HomeServiceSettlementRequestNavigationProp = NativeStackNavigationProp<AppStackParamList, 'HomeServiceSettlementRequest'>;

interface AvailableHomeServiceEarning {
  currency: string;
  amount: number;
...
export function HomeServiceSettlementRequest() {
  const navigation = useNavigation<HomeServiceSettlementRequestNavigationProp>();
  ...
  const loadData = async () => {
    try {
      ...
      const [earningsData, bankAccountsData, walletsData] = await Promise.all([
        settlementService.getAvailableHomeServiceEarnings(),
      ...
      console.error('Error loading home service settlement data:', e);
  ...
  const handleSubmitSettlement = async () => {
    ...
    try {
      ...
      await settlementService.createSettlementRequest({
        ...
        includedServiceBookingIds: (earnings.bookings || [])
          .map(b => b.id || '')
          .filter((id) => !!id),
        totalServiceBookingsCount: ((earnings.bookings || [])
          .map(b => b.id || '')
          .filter((id) => !!id)).length,
      });
      ...
      console.error('Error submitting home service settlement request:', error);
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Property Settlement</Text>
                ...
                {availableEarnings.map((earning) => (
                  ...
                  >
                    ...
                    <Text style={styles.earningRides}>
                      {earning.bookingsCount} booking{earning.bookingsCount !== 1 ? 's' : ''}
                    </Text>

                    {earning.bookings && earning.bookings.length > 0 && (
                      <View style={styles.rideDetailsContainer}>
                        <Text style={styles.rideDetailsTitle}>Booking Details:</Text>
                        {earning.bookings.slice(0, 3).map((b) => (
                          <View key={b.id} style={styles.rideDetailItem}>
                            <Text style={styles.rideDetailText}>
                              {b.bookingRef ? `#${b.bookingRef}` : `#${b.id.slice(0, 8)}`}
                              {b.title ? ` · ${b.title}` : ''} - {formatAmount(b.earnings, earning.currency)}
                            </Text>
                          </View>
                        ))}
                        {earning.bookings.length > 3 && (
                          <Text style={styles.rideDetailText}>+{earning.bookings.length - 3} more bookings</Text>
                        )}
                      </View>
                    )}
                  </TouchableOpacity>
