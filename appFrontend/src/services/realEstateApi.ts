...
export const realEstateApi = {
  getListings: async (params?: {
    ...
    const res = await api.get('/api/property-listings', { params });
  ...
  searchListings: async (params: {
    ...
    const res = await api.get('/api/property-listings/search', { params });
    return res.data?.data ?? [];
  },

  getFeatured: async (limit = 6): Promise<PropertyListing[]> => {
    const res = await api.get('/api/property-listings/featured', { params: { limit } });
    return res.data?.data ?? [];
  },

  getListing: async (id: string): Promise<PropertyListing> => {
    const res = await api.get(`/api/property-listings/${id}`);
    return res.data?.data;
  },
  ...
  getAvailability: async (listingId: string, params: {
    ...
    const res = await api.get(`/api/property-listings/${listingId}/availability`, { params });
  ...
  applyAsAgent: async (data: PropertyAgentApplicationPayload) => {
    const res = await api.post('/api/property-agents/apply', data);
    return res.data;
  },

  getMyApplication: async () => {
    const res = await api.get('/api/property-agents/application/me');
    return res.data?.data;
  },

  createListing: async (data: PropertyListingPayload) => {
    const res = await api.post('/api/property-listings', data);
    return res.data?.data;
  },

  updateListing: async (id: string, data: Partial<PropertyListingPayload & { status: string }>) => {
    const res = await api.patch(`/api/property-listings/${id}`, data);
    return res.data?.data;
  },

  publishListing: async (id: string): Promise<PropertyListing> => {
    const res = await api.post(`/api/property-listings/${id}/publish`);
    return res.data?.data;
  },

  deactivateListing: async (id: string) => {
    await api.delete(`/api/property-listings/${id}`);
  },

  getMyListings: async (): Promise<PropertyListing[]> => {
    const res = await api.get('/api/property-listings/agent/mine');
    return res.data?.data ?? [];
  },

  getRoomTypes: async (listingId: string): Promise<PropertyRoomType[]> => {
    const res = await api.get(`/api/property-listings/${listingId}/room-types`);
    return res.data?.data ?? [];
  },

  createRoomType: async (listingId: string, data: Partial<PropertyRoomType>) => {
    const res = await api.post(`/api/property-listings/${listingId}/room-types`, data);
    return res.data?.data;
  },

  updateRoomType: async (listingId: string, roomTypeId: string, data: Partial<PropertyRoomType>) => {
    const res = await api.patch(`/api/property-listings/${listingId}/room-types/${roomTypeId}`, data);
    return res.data?.data;
  },

  deleteRoomType: async (listingId: string, roomTypeId: string) => {
    await api.delete(`/api/property-listings/${listingId}/room-types/${roomTypeId}`);
  },

  getBlockedDates: async (listingId: string): Promise<PropertyBlockedDate[]> => {
    const res = await api.get(`/api/property-listings/${listingId}/blocked-dates`);
    return res.data?.data ?? [];
  },

  addBlockedDate: async (listingId: string, data: { startDate: string; endDate: string; roomTypeId?: string; reason?: string }) => {
    const res = await api.post(`/api/property-listings/${listingId}/blocked-dates`, data);
    return res.data?.data;
  },

  deleteBlockedDate: async (listingId: string, blockId: string) => {
    await api.delete(`/api/property-listings/${listingId}/blocked-dates/${blockId}`);
  },

  createBooking: async (data: PropertyBookingPayload) => {
    const res = await api.post('/api/property-bookings', data);
    return res.data?.data;
  },

  createInquiry: async (data: Record<string, unknown>) => {
    const res = await api.post('/api/property-bookings/inquiries', data);
    return res.data?.data;
  },

  getMyBookings: async (): Promise<PropertyBooking[]> => {
    const res = await api.get('/api/property-bookings/mine');
    return res.data?.data ?? [];
  },

  getAgentInbox: async (): Promise<{ bookings: PropertyBooking[]; inquiries: PropertyInquiry[] }> => {
    const res = await api.get('/api/property-bookings/agent/mine');
    return res.data?.data;
  },

  updateBookingStatus: async (id: string, status: string) => {
    const res = await api.patch(`/api/property-bookings/${id}/status`, { status });
    return res.data?.data;
  },

  getMyInquiries: async (): Promise<PropertyInquiry[]> => {
    const res = await api.get('/api/property-bookings/inquiries/mine');
    return res.data?.data ?? [];
  },
