...
import {
  settlementService,
  type AvailableHomeServiceEarnings,
  type BankAccount,
  type Wallet,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await settlementService.createSettlementRequest({
        ...
        includedServiceBookingIds: ids,
        totalServiceBookingsCount: ids.length,
      });
      alert('Settlement request submitted successfully.');
      navigate('/settlement-history?channel=HOME_SERVICES');
    } catch (err: any) {
    ...
    <div className="max-w-2xl mx-auto bg-white min-h-full">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Home services earnings</p>
        </div>
      </div>
      ...
      <div className="p-4 space-y-5">
        ...
        <section>
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              className="text-sm text-sky-600 font-medium flex items-center gap-1"
            >
          ...
          <div className="grid grid-cols-2 gap-2 mb-3">
            <button
              ...
              className={`p-3 rounded-xl border flex items-center gap-2 ${
                selectedPaymentMethod === 'BANK_TRANSFER' ? 'border-sky-500 bg-sky-50' : 'border-gray-200'
              }`}
            >
            ...
            <button
              ...
              className={`p-3 rounded-xl border flex items-center gap-2 ${
                selectedPaymentMethod === 'WALLET_TRANSFER' ? 'border-sky-500 bg-sky-50' : 'border-gray-200'
              }`}
            >
                ...
                bankAccounts.map((account) => (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-lg border ${
                      selectedBankAccount === account.id ? 'border-sky-500 bg-sky-50' : 'border-gray-200'
                    }`}
                  >
              ...
              {wallets.length === 0 ? (
                ...
                wallets.map((wallet) => (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-lg border ${
                      selectedWallet === wallet.id ? 'border-sky-500 bg-sky-50' : 'border-gray-200'
                    }`}
                  >
        ...
        <button
          ...
          className="w-full py-3 rounded-xl bg-sky-600 text-white font-semibold disabled:opacity-50"
        >
          ...
          <div className="relative z-10 w-full max-w-md bg-white rounded-t-2xl sm:rounded-2xl p-4 space-y-3">
            ...
            {!methodType ? (
              ...
              <div className="space-y-2">
                ...
                <button type="button" onClick={handleAddMethod} className="w-full py-2.5 bg-sky-600 text-white rounded-xl font-semibold">Save bank</button>
              </div>
            ) : (
              <div className="space-y-2">
                ...
                <button type="button" onClick={handleAddMethod} className="w-full py-2.5 bg-sky-600 text-white rounded-xl font-semibold">Save wallet</button>
              </div>
            )}
          </div>
        </div>
