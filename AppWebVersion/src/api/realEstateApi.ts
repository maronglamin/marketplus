...
export const realEstateApi = {
  ...
  getInquiry: async (id: string): Promise<PropertyInquiry> => {
    const res = await getApi().get(`/property-bookings/inquiries/${id}`);
    return res.data?.data;
  },

  updateInquiryStatus: async (id: string, status: string) => {
    const res = await getApi().patch(`/property-bookings/inquiries/${id}/status`, { status });
    return res.data?.data;
  },
  ...
  processInquiryPayment: async (id: string, paymentMethodId: string, paymentIntentId?: string) => {
    const res = await getApi().post(`/property-bookings/inquiries/${id}/payment`, {
      paymentMethodId,
      paymentIntentId,
    });
    return res.data;
